...
#'
#' This function is a vectorized version of \code{which.in}, which returns the index of the value that is the minimum (or the first mimumal value, if there is a tie). In this case, the function is supplied two or more vectors of the same length. For each element at the same position (e.g., the first element in each vector, then the second element, etc.) the function returns an integer indicating which vector has the lowest value (or the index of the first vector with the lowest value in case of ties).
...
#' @return Vector the same length as the input, with numeric values indicating which vector has the lowest value at that position. In case of ties, the index of the first vector is returned.
#' @seealso \code{\link{which.min}}, \code{\link{which.max}}, \code{\link{whichPMax}}, \code{\link{pmin}}, \code{\link{pmax}}
#' @examples
#' set.seed(123)
#' a <- sample(9, 5)
#' b <- sample(9, 5)
